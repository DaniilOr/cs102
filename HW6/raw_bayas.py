...
class NaiveBayesClassifier:
    ...
    def remove_stopwords(self, text):
        ...
        return [word for word in text if not word in self.stop_words]

    def lemmatization(self, text):
        return [self.morph.parse(word)[0].normal_form for word in text]

    def clean(self, X):
        filtred_x = []
        for vector in X:
            filtred_x.append(self.lemmatization(self.remove_stopwords(vector)))
        return filtred_x

    def fit(self, X, y):
        """ Fit Naive Bayes classifier according to X, y. """
        filtred_x = self.clean(X)
        for cl in y:
    ...
    def predict(self, X):
        ...
        for i in range(len(filtred_x)):
            sums = []
            sums.extend([log(1 / len(self.coder.keys()))] *
                                       len(self.coder.keys()))
